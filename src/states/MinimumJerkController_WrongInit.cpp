...
void MinimumJerkController_WrongInit::start(mc_control::fsm::Controller & ctl_)
{
  ...
  initPos_ = robot.bodyPosW("DS4_tool").translation();
  ...
  ctl.minJerkTask->LQR_R(1);
  ctl.minJerkTask->W_e(Eigen::Vector3d({1, 1, 1}));
  ctl.minJerkTask->W_u(Eigen::Vector4d(1, 200, 200, 100));
  ...
  oriTask_ =
      std::make_shared<mc_tasks::OrientationTask>("DS4_tool", ctl.robots(), ctl.robot().robotIndex(), 20.0, 1000.0);
